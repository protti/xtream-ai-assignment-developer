...
class Predict(Resource):
    ...
    def put(self):
        ...
        request_data = Observability(type="request", operation="predict_price", request=str(data))
        ...
        response_data = Observability(type="response", operation="predict_price", response=str(predicted_value))
...
class Closer(Resource):
    ...
    def put(self):
        ...
        request_data = Observability(type="request", operation="find_closer", request=str(data))
        db.session.add(request_data)
        db.session.commit()
        ...
        dict_similar = similar_samples.to_dict(orient='records')

        response_data = Observability(type="response", operation="find_closer", response=str(dict_similar))
        db.session.add(response_data)
        db.session.commit()

        return {"closer": dict_similar}
